...
const getWalletPath = async (org)=> {
    console.log('Inside getWalletPath');
    let walletPath;
    if (org == "Org1") {
...
const getAffiliation = async (org) => {
    console.log('Inside getAffiliation');
    if(org == "Org1"){
...
const getRegisteredUser = async (username, userOrg) => {
    ...
    let ccp = await getCCP(userOrg); //connection Path
    const caURL = await getCaUrl(userOrg,ccp);  //connection url from connection json
    const ca = new FabricCAServices(caURL);
    const walletPath = await getWalletPath(userOrg);  //different wallet storage for different org users
    const wallet = await Wallets.newFileSystemWallet(walletPath);
    console.log(`Wallet path: ${walletPath}`);

    const userIdentity = await wallet.get(username); //checking for existance of username
    if(userIdentity){
    ...
    try {
        ...
        console.log('Error occured at affilication:  ', error);
...
const enrollAdmin = async (org, ccp) => {
    ...
    try{
        const caInfo = await getCaInfo(org,ccp); //ccp.certifiedAuthorities['ca.org1.example.com'];
...
const isUserRegistered = async (username, userOrg) => {
    console.log('Inside isUserRegistered');
