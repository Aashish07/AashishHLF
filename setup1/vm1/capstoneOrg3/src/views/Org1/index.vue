<template lang="pug">
    v-app
      ...
      v-navigation-drawer(app floating)
          v-list-item
            v-list-item-content
              v-list-item-title(class="title") Organisation 3
          ...
          v-list(
            ...
            v-list-item(link to='/Org2')
              v-list-item-content Organisation2
      ...
      div
        v-data-table(
          ...
          template(v-slot:item.actions="{ item }")
            router-link(:to="'/updateOrg1/' + item.tradeId") Update
      v-content
        v-container(fluid)
...
export default {
  ...
  computed: {
    ...
    methods: {
      async fetchdata(){
        await this.$store.dispatch('fetchOrg2trades');
      }
    },
    mounted(){
      this.fetchdata();
